...
{	
	...
	function JnPay()
	{
		...
		$indata['bankNo'] = $bank_card->CreditNumber;//信用卡号
		$user_contact = DB::table('xyk_usercontact')->where('UserId',$this->user['UserId'])->where('Isvalid',1)->select('CertNo','Contact')->first();
		if(!$user_contact){
			throw new Exception("用户身份证件不存在", 1202);
		}
		$indata['bankCert'] = $user_contact->CertNo;//身份证号
		$indata['linkId'] =  date("YmdHis") . rand(1000, 9999);;//订单流水号
		$indata['orderType'] = 10;//实时到账
		$indata['amount'] = $amount;//消费金额
		//$indata['bankPhone'] = $this->user['Mobile'];//绑定手机号码
		$indata['bankPhone'] = 15998571677;
		$indata['bankCvv'] = substr($indata['bankNo'], -3, 3) ;//信用卡后三位
		$indata['bankYxq'] = explode('/',$bank_card->YN)[1] .explode('/',$bank_card->YN)[0];//信用卡有效期
		$indata['bankAccount'] = $user_contact->Contact;//信用卡账户
		$indata['goodsName'] = '充值';//商品名称
		$indata['notifyUrl'] = 'http://api.wlxkd.com/pay/jncb';//回调地址

		$jn_model = new JieniuPay('SdkNocardOrderPayNoSms');
		$jn_model->merchantInit();
		$jn_model->postUrl($indata);
	...
	function getJncb(){
		...
		$result = $_GET;
		$bill_detail = DB::table('xyk_billdetails')->where('OrderNum',$result['linkId'])->select('BillId')->first();
		$fee = DB::table('xyk_fee')->first();
		$pay_fee = $bill_detail->Amount * $fee->PayFee / 100;
		$pay_fee = round($pay_fee, 2);

		if ($result['orderStatus'] == 20) {
			Bill::billUpdate($bill_detail->BillId, 'SUCCESS'); // 成功
			// 添加余额 扣除手续废
			$d_money = $bill_detail->Amount - $pay_fee;
			Profit::doProfit($this->user->UserId, $bill_detail->Amount);
			User::where('UserId', $this->user->UserId)->increment('Account', (float)$d_money);
		} else {
			Bill::billUpdate($bill_detail->BillId, 'FAIL');
		}
	}
	...
	public function JnRepay()
	{
		try {
			...
			$bank_card = BankdCard::where('UserId', $this->user->UserId)->where('Id', $this->data['bank_id'])->first();
			$fee = DB::table('xyk_fee')->first();
			$user_contact = DB::table('xyk_usercontact')->where('UserId', $this->user->UserId)->first();
			if (!$fee) {
				throw new Exception("商家未设置费率", 3001);
			}
			...
			$data['amount'] = $money;  //结算到账金额
			$data['bankNo'] = $bank_card->CreditNumber;  //结算银行卡号
			$data['bankAccount'] = $user_contact->Contact;  //结算银行账户
			...
			$data['notifyUrl'] = 'http://api.wlxkd.com/pay/jnrecb';  //支付结果回调地址
	...
	function getJnrecb(){
		...
		if ($result['settleStatus'] == 20) {
			Bill::billUpdate($bill_detail->BillId, 'SUCCESS');
		} else {
			Bill::billUpdate($bill_detail->BillId, 'FAIL');
			// 还回余额
			User::where('UserId', $this->user->UserId)->increment('Account', $bill_detail->Amount);
		}
	}
	...
	function getPlancb(){
		...
		if ($result['settleStatus'] == 20) {
			Bill::billUpdate($bill_detail->BillId, 'SUCCESS');
			PlanDetail::where('Id', $bill_detail->Id)->update(array('status'=> 1));
		} else {
//			Bill::billUpdate($bill_detail->BillId, 'FAIL');
//			// 还回余额
//			User::where('UserId', $this->user->UserId)->increment('Account', $bill_detail->Amount);
